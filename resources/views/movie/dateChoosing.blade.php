...
.custom-form {
  ...
  <div class="cellphone-container">    
      ...
      <form method="post" action="/movies/showtime" class="custom-form">
          @csrf
          <input type="hidden" value="{{$movie->movieId}}" name="movieId"/>
         <input type="hidden" value="{{$p->showtimeid}}" name="showtimeId"/>
         <input type="hidden" value="{{$p->date}}" name="date"/>
         
                <div class="col7"> <div class="time"><button type="submit" class="link">{{ $p->date }}</button></div></div> 
